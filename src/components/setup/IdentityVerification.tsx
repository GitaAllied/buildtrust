...
type DocumentType = 'id' | 'cac' | 'selfie' | 'passport' | 'idCard';
...
const IdentityVerification = ({ data, onChange, userType = 'developer' }: IdentityVerificationProps) => {
  const [files, setFiles] = useState(() => {
    // Try to load from localStorage first (only metadata, not actual files)
    const savedData = localStorage.getItem(IDENTITY_STORAGE_KEY);
    if (savedData) {
      try {
        const parsed = JSON.parse(savedData);
        // Remove any file objects from saved data, keep only metadata
        const cleanedData = {};
        Object.keys(parsed).forEach(key => {
          if (parsed[key] && typeof parsed[key] === 'object' && parsed[key].name) {
            // Keep metadata but remove the actual file object
            const { file, ...metadata } = parsed[key];
            cleanedData[key] = metadata;
          } else {
    ...
    return {
      ...
      passport: null as FileData | null,
      idCard: null as FileData | null,
      ...data
    };
  });
  ...
  // Save to localStorage whenever files change (store metadata only, not actual files)
  useEffect(() => {
    ...
    // Create a copy without the actual file objects for localStorage
    const storageData = {};
    Object.keys(files).forEach(key => {
      if (files[key] && typeof files[key] === 'object') {
        const { file, ...metadata } = files[key];
        storageData[key] = metadata;
      } else {
        storageData[key] = files[key];
      }
    });
  ...
  const handleFileUpload = (type: DocumentType, fileOrList: File | FileList | null) => {
    ...
    if (type === 'selfie' || type === 'passport') {
      if (!file.type.startsWith('image/')) {
        console.warn(`Invalid type for ${type}: ${file.type}`);
        setError(`${type === 'selfie' ? 'Selfie' : 'Passport photo'} must be an image file (JPG, PNG, etc)`);
        return;
      }
    } else if (type === 'id' || type === 'cac' || type === 'idCard') {
    ...
    console.log(`🔍 CURRENT FILES STATE after ${type} upload:`, {
      passport: updatedFiles.passport ? { name: updatedFiles.passport.name, hasFile: !!updatedFiles.passport.file } : null,
      idCard: updatedFiles.idCard ? { name: updatedFiles.idCard.name, hasFile: !!updatedFiles.idCard.file } : null
    });
  };
  ...
  const capturePhoto = async (type: DocumentType) => {
  ...
  const isComplete = userType === 'client' 
    ? (files.passport !== null && files.idCard !== null)
    : (files.id !== null && files.cac !== null && files.selfie !== null);
  ...
  const renderUploadSection = (
    ...
    return (
      <div key={type} className="flex flex-col h-full gap-4">
        ...
        >
          {!fileData ? (
            // If selfie or passport and camera active, show live preview + capture controls
            (type === 'selfie' || type === 'passport') && cameraActive ? (
              ...
              <label htmlFor={inputId} className="cursor-pointer block w-full">
                ...
                {type === 'selfie' || type === 'passport' ? 'Image files up to 10MB' : 'PDF, JPG, PNG, DOC, DOCX up to 10MB'}
                {type === 'selfie' || type === 'passport' && (
  ...
  return (
    <div className="space-y-8">
      <div>
        <h2 className="text-lg font-semibold text-gray-900 mb-2">{userType === 'client' ? 'Document Upload' : 'Identity Verification'}</h2>
        <p className="text-gray-600">{userType === 'client' ? 'Please upload the required documents to complete your verification.' : 'Upload your identification documents to verify your identity. All documents must be clear and current.'}</p>
      </div>
      ...
      {userType === 'client' && (
        <div className="grid md:grid-cols-2 gap-6">
          {renderUploadSection(
            ...
            "passport",
            "image/*",
            "Upload or take a passport-sized photo"
          )}
          ...
          {renderUploadSection(
            ...
            "idCard",
            "image/*,.pdf,.doc,.docx",
            "Upload a valid ID card (driver's license, national ID, etc.)"
          )}
        </div>
      )}
          ...
          <div>
            ...
            <p className="text-sm text-[#253E44]/70 mt-1">
              <span className="font-semibold">
                {userType === 'client' 
                  ? [files.passport, files.idCard].filter(Boolean).length + '/2'
                  : [files.id, files.cac, files.selfie].filter(Boolean).length + '/3'
                }
              </span> documents uploaded
