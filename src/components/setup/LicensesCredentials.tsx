...
const LicensesCredentials = ({ data, onChange }: LicensesCredentialsProps) => {
  ...
  const licensesInputRef = useRef<HTMLInputElement>(null);
  const certificationsInputRef = useRef<HTMLInputElement>(null);
  const testimonialsInputRef = useRef<HTMLInputElement>(null);
  ...
  const handleFileUpload = (type: string, fileList: FileList | null) => {
    console.log(`handleFileUpload called for ${type}`, fileList);
    setError(null);
    if (fileList && fileList.length > 0) {
      ...
      if (valid.length > 0) {
        const updatedFiles = { 
          ...files, 
          [type]: [...(files[type as keyof typeof files] as File[]), ...valid] 
        };
        console.log(`Updated files for ${type}:`, updatedFiles);
        setFiles(updatedFiles);
        onChange(updatedFiles);
        
        // Reset the input
        const inputRef = type === 'licenses' ? licensesInputRef : type === 'certifications' ? certificationsInputRef : testimonialsInputRef;
        if (inputRef.current) {
  ...
  const UploadSection = ({ 
    ...
    const sectionFiles = files[type as keyof typeof files] as File[];
  ...
  return (
    <div className="space-y-8">
      ...
      <UploadSection
        ...
        inputRef={certificationsInputRef}
      />
